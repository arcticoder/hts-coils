...
% Custom commands
\newcommand{\Tesla}[1]{\SI{#1}{\tesla}}
\newcommand{\Ampere}[1]{\SI{#1}{\ampere}}
\newcommand{\Kelvin}[1]{\SI{#1}{\kelvin}}
\newcommand{\Pascal}[1]{\SI{#1}{\pascal}}
...
\begin{frame}{Critical Current Density: The Key Parameter}
    \begin{block}{Kim Model for REBCO}
        ...
        \begin{itemize}
            \item $\Jc^0 = \SI{300e6}{\ampere\per\meter\squared}$ at self-field, \Kelvin{77}
    ...
    \begin{center}
        Operating at \Kelvin{20} and \Tesla{7}: $\Jc \approx \SI{85}{\mega\ampere\per\meter\squared}$
    \end{center}
</frame>
...
\begin{frame}{Mechanical Stress Analysis}
    \begin{block}{Maxwell Stress Tensor}
        ...
        For \Tesla{7} field, $R = \SI{0.16}{\meter}$, $t = \SI{0.01}{\meter}$:
        \begin{equation}
            \sigma_{\text{hoop}} = \frac{(7)^2 \times 0.16}{2 \times 4\pi \times 10^{-7} \times 0.01} = \SI{178}{\mega\pascal}
        \end{equation}
    \end{block}
    ...
    \begin{columns}
        ...
        \begin{column}{0.4\textwidth}
            \begin{tikzpicture}[scale=0.7]
                ...
                \node at (axis cs:25,320) {\small Operating};
                \node at (axis cs:25,300) {\small Point};
                \end{axis}
            \end{tikzpicture}
        \end{column}
    \end{columns}
...
\begin{frame}{Antimatter Research Applications}
    \begin{block}{ALPHA Experiment at CERN}
        ...
        \begin{itemize}
            \item Magnetic trap depth: $\sim\SI{1}{\kelvin}$
            \item Confinement time: $\sim\SI{1000}{\second}$
...
\begin{frame}{Prototype Specifications}
    \begin{columns}
        \begin{column}{0.5\textwidth}
            ...
            \begin{itemize}
                \item Radius: $R = \SI{0.04}{\meter}$
                \item Current: $I = \Ampere{360}$
                \item Turns: $N = 200$
                \item Field: $B = \Tesla{2.9}$
                \item Tape length: \SI{3.4}{\kilo\meter}
            \end{itemize}
...
\begin{frame}{Exercise 1: Critical Current Calculation}
    \begin{block}{Problem}
        ...
        $\Jc^0 = \SI{300e6}{\ampere\per\meter\squared}$, $T_c = \Kelvin{90}$, $B_0 = \Tesla{5}$, $n = m = 1.5$
    \end{block}
    ...
    \vspace{0.3cm}
    \textbf{Try it yourself!} What current can a \SI{4}{\milli\meter} wide tape carry?
\end{frame}

\begin{frame}{Exercise 2: Magnetic Field Design}
    \begin{block}{Problem}
        Design a single circular coil to generate \Tesla{1} at its center.
        If the coil radius is \SI{0.1}{\meter}, how many ampere-turns are needed?
        What if we want the field uniform to within 1\% over a \SI{1}{\centi\meter} region?
    \end{block}
...
\begin{frame}{Exercise 3: Stress Analysis}
    \begin{block}{Problem}
        A \Tesla{5} magnetic field creates hoop stress in a coil with radius \SI{0.2}{\meter}
        and conductor thickness \SI{5}{\milli\meter}. Calculate the stress and determine
